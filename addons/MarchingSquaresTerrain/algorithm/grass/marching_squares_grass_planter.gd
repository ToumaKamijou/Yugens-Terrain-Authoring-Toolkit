	...
	for i in range(0, len(verts), 3):
		...
		while (point_index < len(points)):
			...
			if u + v <= 1:
				...
				var color_0 = MarchingSquaresTerrainCell.get_dominant_color(colors_0[i]*u + colors_0[i+1]*v + colors_0[i+2]*(1-u-v))
				var color_1 = MarchingSquaresTerrainCell.get_dominant_color(colors_1[i]*u + colors_1[i+1]*v + colors_1[i+2]*(1-u-v))
