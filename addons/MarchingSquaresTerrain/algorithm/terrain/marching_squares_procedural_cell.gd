...
func add_c8(chunk: MarchingSquaresTerrainChunk) -> void:
	...
	add_point(chunk, 1, dy, 1, 0, 0, rotation)
	add_point(chunk, 0.5, dy, 1, 1, 0, rotation)
	add_point(chunk, 1, (by+dy)/2, 0.5, 0, 0, rotation)
	
	# B corner
	add_point(chunk, 1, by, 0, 0, 0, rotation)
	add_point(chunk, 1, (by+dy)/2, 0.5, 0, 0, rotation)
	add_point(chunk, 0.5, by, 0, 0, 1, rotation)
	
	# Center floors
	add_point(chunk, 0.5, by, 0, 0, 1, rotation)
	add_point(chunk, 1, (by+dy)/2, 0.5, 0, 0, rotation)
	add_point(chunk, 0, by, 0.5, 1, 1, rotation)
	...
	chunk.start_wall()
	add_point(chunk, 0, by, 0.5, 0, 0, rotation)
	add_point(chunk, 0.5, dy, 1, 0, 0, rotation)
	add_point(chunk, 0, cy, 0.5, 0, 0, rotation)
	
	add_point(chunk, 0.5, cy, 1, 0, 0, rotation)
	add_point(chunk, 0, cy, 0.5, 0, 0, rotation)
	add_point(chunk, 0.5, dy, 1, 0, 0, rotation)
	
	# C upper floor
	chunk.start_floor()
	add_point(chunk, 0, cy, 1, 0, 0, rotation)
	add_point(chunk, 0, cy, 0.5, 0, 1, rotation)
	add_point(chunk, 0.5, cy, 1, 0, 1, rotation)
...
func add_c9(chunk: MarchingSquaresTerrainChunk) -> void:
	...
	chunk.start_floor()
	add_point(chunk, 1, dy, 1, 0, 0, rotation)
	add_point(chunk, 0.5, (dy + cy) / 2, 1, 0, 0, rotation)
	add_point(chunk, 1, dy, 0.5, 0, 0, rotation)
	...
	# Center floors
	add_point(chunk, 0, cy, 0.5, 0, 0, rotation)
	add_point(chunk, 0.5, cy, 0, 0, 0, rotation)
	add_point(chunk, 0.5, (dy + cy) / 2, 1, 0, 0, rotation)
	...
	chunk.start_wall()
	add_point(chunk, 0.5, cy, 0, 0, 0, rotation)
	add_point(chunk, 0.5, by, 0, 0, 0, rotation)
	add_point(chunk, 1, dy, 0.5, 0, 0, rotation)

	add_point(chunk, 1, by, 0.5, 0, 0, rotation)
	add_point(chunk, 1, dy, 0.5, 0, 0, rotation)
	add_point(chunk, 0.5, by, 0, 0, 0, rotation)
...
func add_c16(chunk: MarchingSquaresTerrainChunk) -> void:
	...
	# Upper floor
	chunk.start_floor()
	add_point(chunk, 0, ay, 0, 0, 0, rotation)
	add_point(chunk, 1, by, 0, 0, 0, rotation)
	add_point(chunk, 1, edge_by, 0.5, 0, 0, rotation)

	add_point(chunk, 1, edge_by, 0.5, 0, 1, rotation)
	add_point(chunk, 0, ay, 0.5, 0, 1, rotation)
	add_point(chunk, 0, ay, 0, 0, 0, rotation)
	...
	add_point(chunk, 0, cy, 0.5, 0, 0, rotation)
	add_point(chunk, 0, ay, 0.5, 0, 1, rotation)
	add_point(chunk, 1, edge_dy, 0.5, 1, 0, rotation)

	# Lower floor
	chunk.start_floor()
	add_point(chunk, 0, cy, 0.5, 1, 0, rotation)
	add_point(chunk, 1, edge_dy, 0.5, 1, 0, rotation)
	add_point(chunk, 0, cy, 1, 0, 0, rotation)

	add_point(chunk, 1, dy, 1, 0, 0, rotation)
	add_point(chunk, 0, cy, 1, 0, 0, rotation)
	add_point(chunk, 1, edge_dy, 0.5, 0, 0, rotation)
...
func add_c17(chunk: MarchingSquaresTerrainChunk) -> void:
	...
	add_point(chunk, 0, ay, 0, 0, 0, rotation)
	add_point(chunk, 1, by, 0, 0, 0, rotation)
	add_point(chunk, 0, edge_ay, 0.5, 0, 0, rotation)
	
	add_point(chunk, 1, by, 0.5, 0, 1, rotation)
	add_point(chunk, 0, edge_ay, 0.5, 0, 1, rotation)
	add_point(chunk, 1, by, 0, 0, 0, rotation)
	
	# Wall from left to right edge
	chunk.start_wall()
	add_point(chunk, 1, by, 0.5, 1, 1, rotation)
	add_point(chunk, 1, dy, 0.5, 1, 0, rotation)
	add_point(chunk, 0, edge_ay, 0.5, 0, 0, rotation)
	
	# Lower floor - use C and D edge
	chunk.start_floor()
	add_point(chunk, 0, edge_cy, 0.5, 1, 0, rotation)
	add_point(chunk, 1, dy, 0.5, 1, 0, rotation)
	add_point(chunk, 1, dy, 1, 0, 0, rotation)
	
	add_point(chunk, 0, cy, 1, 0, 0, rotation)
	add_point(chunk, 0, edge_cy, 0.5, 0, 0, rotation)
	add_point(chunk, 1, dy, 1, 0, 0, rotation)
	
func add_point(chunk: MarchingSquaresTerrainChunk, x: float, y: float, z: float, u: float, v: float, ro: CellRotation) -> void:
	chunk.add_point(Vector3(x ,y ,z), Vector2(u, v), Vector3.ZERO, ro)
...
func add_outer_corner(chunk: MarchingSquaresTerrainChunk, floor_below: bool = true, floor_above: bool = true, flatten_bottom: bool = false, bottom_height: float = -1):
	...
	if floor_above:
		chunk.start_floor()
		add_point(chunk, 0, ay, 0, 0, 0, rotation)
		add_point(chunk, 0.5, ay, 0, 0, 1, rotation)
		add_point(chunk, 0, ay, 0.5, 0, 1, rotation)
	...
	add_point(chunk, 0, edge_cy, 0.5, 0, 0, rotation)
	add_point(chunk, 0, ay, 0.5, 0, 1, rotation)
	add_point(chunk, 0.5, edge_by, 0, 1, 0, rotation)
	
	add_point(chunk, 0.5, ay, 0, 1, 1, rotation)
	add_point(chunk, 0.5, edge_by, 0, 1, 0, rotation)
	add_point(chunk, 0, ay, 0.5, 0, 1, rotation)
	...
	if floor_below:
		...
		add_point(chunk, 0, cy, 1,0,0, rotation)
		add_point(chunk, 0, cy, 0.5, 1, 0, rotation)
		add_point(chunk, 0.5, by, 0, 1, 0, rotation)
...
func add_edge(chunk: MarchingSquaresTerrainChunk, floor_below: bool, floor_above: bool, a_x: float = 0, b_x: float = 1):
	...
	if floor_above:
		chunk.start_floor()
		add_point(chunk, a_x, edge_ay, 0, 1 if a_x > 0 else 0, 0, rotation)
		add_point(chunk, b_x, edge_by, 0, 1 if b_x < 1 else 0, 0, rotation)
		add_point(chunk, 0, edge_ay, 0.5, -1 if b_x < 1 else (1 if a_x > 0 else 0), 1, rotation)
		
		add_point(chunk, 1, edge_by, 0.5, -1 if a_x > 0  else (1 if b_x < 1 else 0), 1, rotation)
		add_point(chunk, 0, edge_ay, 0.5, -1 if b_x < 1 else (1 if a_x > 0 else 0), 1, rotation)
		add_point(chunk, b_x, edge_by, 0, 1 if b_x < 1 else 0, 0, rotation)
	...
	add_point(chunk, 1, edge_by, 0.5, 1, 1, rotation)
	add_point(chunk, 1, edge_dy, 0.5, 1, 0, rotation)
	add_point(chunk, 0, edge_ay, 0.5, 0, 1, rotation)
...
func add_inner_corner(chunk: MarchingSquaresTerrainChunk, lower_floor: bool = true, full_upper_floor: bool = true, flatten: bool = false, bd_floor: bool = false, cd_floor: bool = false):
	...
	if lower_floor:
		chunk.start_floor()
		add_point(chunk, 0, ay, 0, 0, 0, rotation)
		add_point(chunk, 0.5, ay, 0, 1, 0, rotation)
		add_point(chunk, 0, ay, 0.5, 1, 0, rotation)

	chunk.start_wall()
	add_point(chunk, 0, ay, 0.5, 1, 0, rotation)
	add_point(chunk, 0.5, ay, 0, 0, 0, rotation)
	add_point(chunk, 0, corner_cy, 0.5, 1, 1, rotation)
	
	add_point(chunk, 0.5, corner_by, 0, 0, 1, rotation)
	add_point(chunk, 0, corner_cy, 0.5, 1, 1, rotation)
	add_point(chunk, 0.5, ay, 0, 0, 0, rotation)
	...
	if full_upper_floor:
		add_point(chunk, 1, dy, 1, 0, 0, rotation)
		add_point(chunk, 0, corner_cy, 1, 0, 0, rotation)
		add_point(chunk, 1, corner_by, 0, 0, 0, rotation)
		
		add_point(chunk, 0, corner_cy, 1, 0, 0, rotation)
		add_point(chunk, 0, corner_cy, 0.5, 0, 1, rotation)
		add_point(chunk, 0.5, corner_by, 0, 0, 1, rotation)
	...
	if bd_floor: 
		add_point(chunk, 0, cy, 0.5, 0, 1, rotation)
		add_point(chunk, 0.5, cy, 0, 1, 1, rotation)
		add_point(chunk, 0, cy, 1, 0, 0, rotation)
		
		add_point(chunk, 0.5, cy, 1, 1, -1, rotation)
		add_point(chunk, 0, cy, 1, 0, 0, rotation)
		add_point(chunk, 0.5, cy, 0, 1, 1, rotation)
...
func add_diagonal_floor(chunk: MarchingSquaresTerrainChunk, b_y: float, c_y: float, rot: CellRotation, a_cliff: bool, d_cliff: bool):
	...
	add_point(chunk, 1, b_y, 0, 0 ,0 , rot)
	add_point(chunk, 0, c_y, 1, 0 ,0 , rot)
	add_point(chunk, 0.5, b_y, 0, 0 if a_cliff else 1, 1 if a_cliff else 0, rot)
	
	add_point(chunk, 0, c_y, 1, 0, 0,  rot)
	add_point(chunk, 0, c_y, 0.5, 0 if a_cliff else 1, 1 if a_cliff else 0, rot)
	add_point(chunk, 0.5, b_y, 0, 0 if a_cliff else 1, 1 if a_cliff else 0, rot)
	
	add_point(chunk, 1, b_y, 0, 0 ,0 , rot)
	add_point(chunk, 1, b_y, 0.5, 0 if d_cliff else 1, 1 if d_cliff else 0, rot)
	add_point(chunk, 0, c_y, 1, 0 ,0 , rot)
	
	add_point(chunk, 0, c_y, 1, 0, 0, rot)
	add_point(chunk, 1, b_y, 0.5, 0 if d_cliff else 1, 1 if d_cliff else 0, rot)
	add_point(chunk, 0.5, c_y, 1, 0 if d_cliff else 1, 1 if d_cliff else 0, rot)
