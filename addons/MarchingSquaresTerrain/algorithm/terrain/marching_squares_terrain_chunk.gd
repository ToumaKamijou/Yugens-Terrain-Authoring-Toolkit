...
func generate_terrain_cells():
	...
	for z in range(dimensions.z - 1):
		for x in range(dimensions.x - 1):
			...
			var cell_data := CellData.new(height_map[z][x], height_map[z][x+1], height_map[z+1][x], height_map[z+1][x+1], merge_threshold)
			
			# Case 0
			# If all edges are connected, put a full floor here.
			if cell_data.all_edges_are_connected():
				add_full_floor(cell_data)
			...
			for rot in range(4):
				...
				if cell_data.is_higher(cell_data.ay, cell_data.by) and cell_data.is_higher(cell_data.ay, cell_data.cy) and cell_data.bd and cell_data.cd:
					add_outer_corner(cell_data, true, true)
				...
				elif cell_data.is_higher(cell_data.ay, cell_data.cy) and cell_data.is_higher(cell_data.by, cell_data.dy) and cell_data.ab and cell_data.cd:
					add_edge(cell_data, true, true)
				
				# Case 3: AB edge with A outer corner above
				elif cell_data.is_higher(cell_data.ay, cell_data.by) and cell_data.is_higher(cell_data.ay, cell_data.cy) and cell_data.is_higher(cell_data.by, cell_data.dy) and cell_data.cd:
					add_edge(cell_data, true, true, 0.5, 1)
					add_outer_corner(cell_data, false, true, true, cell_data.by)
				
				# Case 4: AB edge with B outer corner above
				elif cell_data.is_higher(cell_data.by, cell_data.ay) and cell_data.is_higher(cell_data.ay, cell_data.cy) and cell_data.is_higher(cell_data.by, cell_data.dy) and cell_data.cd:
					add_edge(cell_data, true, true, 0, 0.5)
					cell_data.rotate(1)
					add_outer_corner(cell_data, false, true, true, cell_data.cy)
				
				# Case 5: B and C are higher than A and D.
				# Diagonal raised floor between B and C.
				# B and C must be within merge distance.
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_lower(cell_data.dy, cell_data.by) and cell_data.is_lower(cell_data.dy, cell_data.cy) and cell_data.is_merged(cell_data.by, cell_data.cy):
					add_inner_corner(cell_data, true, false)
					add_diagonal_floor(cell_data.by, cell_data.cy, cell_data.rotation, true, true)
					cell_data.rotate(2)
					add_inner_corner(cell_data, true, false)
				...
				# Place a raised diagonal floor between, and an outer corner around B.
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_lower(cell_data.dy, cell_data.by) and cell_data.is_lower(cell_data.dy, cell_data.cy) and cell_data.is_higher(cell_data.by, cell_data.cy):
					add_inner_corner(cell_data,true, false, true)
					add_diagonal_floor(cell_data.cy, cell_data.cy, cell_data.rotation, true, true)
					...
					# higher corner B
					cell_data.rotate(-1)
					add_outer_corner(cell_data, false, true)
				
				# Case 6: inner corner, where A is lower than B and C, and D is connected to B and C.
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.bd and cell_data.cd:
					add_inner_corner(cell_data,true, true)
				...
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_higher(cell_data.dy, cell_data.by) and cell_data.is_higher(cell_data.dy, cell_data.cy) and cell_data.is_merged(cell_data.by, cell_data.cy):
					add_inner_corner(cell_data,true, false)
					add_diagonal_floor(cell_data.by, cell_data.cy, cell_data.rotation, true, false)
					cell_data.rotate(2)
					add_outer_corner(cell_data,false, true)
				
				# Case 8: Inner corner surrounding A, with an outer corner sitting atop C.
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_lower(cell_data.dy, cell_data.cy) and cell_data.bd:
					add_inner_corner(cell_data,true, false, true)
					start_floor()
					
					# D corner. B edge is connected, so use halfway point bewteen B and D
					add_point(1, cell_data.dy, 1, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.dy, 1, 1, 0, cell_data.rotation)
					add_point(1, (cell_data.by+cell_data.dy)/2, 0.5, 0, 0, cell_data.rotation)
					
					# B corner
					add_point(1, cell_data.by, 0, 0, 0, cell_data.rotation)
					add_point(1, (cell_data.by+cell_data.dy)/2, 0.5, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.by, 0, 0, 1, cell_data.rotation)
					
					# Center floors
					add_point(0.5, cell_data.by, 0, 0, 1, cell_data.rotation)
					add_point(1, (cell_data.by+cell_data.dy)/2, 0.5, 0, 0, cell_data.rotation)
					add_point(0, cell_data.by, 0.5, 1, 1, cell_data.rotation)
					...
					start_wall()
					add_point(0, cell_data.by, 0.5, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.dy, 1, 0, 0, cell_data.rotation)
					add_point(0, cell_data.cy, 0.5, 0, 0, cell_data.rotation)
					
					add_point(0.5, cell_data.cy, 1, 0, 0, cell_data.rotation)
					add_point(0, cell_data.cy, 0.5, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.dy, 1, 0, 0, cell_data.rotation)
					
					# C upper floor
					start_floor()
					add_point(0, cell_data.cy, 1, 0, 0, cell_data.rotation)
					add_point(0, cell_data.cy, 0.5, 0, 1, cell_data.rotation)
					add_point(0.5, cell_data.cy, 1, 0, 1, cell_data.rotation)
				
				# Case 9: Inner corner surrounding A, with an outer corner sitting atop B.
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_lower(cell_data.dy, cell_data.by) and cell_data.cd:
					add_inner_corner(cell_data, true, false, true)
					
					# D corner. C edge is connected, so use halfway point bewteen C and D
					start_floor()
					add_point(1, cell_data.dy, 1, 0, 0, cell_data.rotation)
					add_point(0.5, (cell_data.dy + cell_data.cy) / 2, 1, 0, 0, cell_data.rotation)
					add_point(1, cell_data.dy, 0.5, 0, 0, cell_data.rotation)
					...
					# Center floors
					add_point(0, cell_data.cy, 0.5, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.cy, 0, 0, 0, cell_data.rotation)
					add_point(0.5, (cell_data.dy + cell_data.cy) / 2, 1, 0, 0, cell_data.rotation)
					...
					start_wall()
					add_point(0.5, cell_data.cy, 0, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.by, 0, 0, 0, cell_data.rotation)
					add_point(1, cell_data.dy, 0.5, 0, 0, cell_data.rotation)

					add_point(1, cell_data.by, 0.5, 0, 0, cell_data.rotation)
					add_point(1, cell_data.dy, 0.5, 0, 0, cell_data.rotation)
					add_point(0.5, cell_data.by, 0, 0, 0, cell_data.rotation)
				...
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_higher(cell_data.dy, cell_data.cy) and cell_data.bd:
					add_inner_corner(cell_data, true, false, true, true, false)
					cell_data.rotate(1)
					add_edge(cell_data, false, true)

				# Case 11: Inner corner surrounding A, with an edge sitting atop CD.
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_higher(cell_data.dy, cell_data.by) and cell_data.cd:
					add_inner_corner(cell_data, true, false, true, false, true)
					cell_data.rotate(2)
					add_edge(cell_data, false, true)
				...
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.by, cell_data.dy) and cell_data.is_lower(cell_data.dy, cell_data.cy) and cell_data.is_higher(cell_data.cy, cell_data.ay):
					add_inner_corner(cell_data, true, false, true, false, true)
					cell_data.rotate(2)
					add_edge(cell_data, false, true, 0, 0.5)
					cell_data.rotate(1)
					add_outer_corner(cell_data, false, true, true, cell_data.cy)

				# Case 13: Clockwise upwards spiral, A lowest and B highest
				elif cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_lower(cell_data.cy, cell_data.dy) and cell_data.is_lower(cell_data.dy, cell_data.by) and cell_data.is_higher(cell_data.by, cell_data.ay):
					add_inner_corner(cell_data, true, false, true, true, false)
					cell_data.rotate(1)
					add_edge(cell_data, false, true, 0.5, 1)
					add_outer_corner(cell_data, false, true, true, cell_data.by)

				# Case 14: A<B, B<C, C<D
				elif cell_data.is_lower(cell_data.ay, cell_data.by) and cell_data.is_lower(cell_data.by, cell_data.cy) and cell_data.is_lower(cell_data.cy, cell_data.dy):
					add_inner_corner(cell_data, true, false, true, false, true)
					cell_data.rotate(2)
					add_edge(cell_data, false, true, 0.5, 1)
					add_outer_corner(cell_data, false, true, true, cell_data.by)
				...
				# Case 15: A<C, C<B, B<D
				elif cell_data.is_lower(cell_data.ay, cell_data.cy) and cell_data.is_lower(cell_data.cy, cell_data.by) and cell_data.is_lower(cell_data.by, cell_data.dy):
					add_inner_corner(cell_data, true, false, true, true, false)
					cell_data.rotate(1)
					add_edge(cell_data, false, true, 0, 0.5)
					cell_data.rotate(1)
				...
				# Case 16: All edges are connected, except AC, and A is higher than C.
				elif cell_data.ab and cell_data.bd and cell_data.cd and cell_data.is_higher(cell_data.ay, cell_data.cy):
					var edge_by = (cell_data.by + cell_data.dy) / 2
					var edge_dy = (cell_data.by + cell_data.dy) / 2

					# Upper floor
					start_floor()
					add_point(0, cell_data.ay, 0, 0, 0, cell_data.rotation)
					add_point(1, cell_data.by, 0, 0, 0, cell_data.rotation)
					add_point(1, edge_by, 0.5, 0, 0, cell_data.rotation)

					add_point(1, edge_by, 0.5, 0, 1, cell_data.rotation)
					add_point(0, cell_data.ay, 0.5, 0, 1, cell_data.rotation)
					add_point(0, cell_data.ay, 0, 0, 0, cell_data.rotation)
					...
					add_point(0, cell_data.cy, 0.5, 0, 0, cell_data.rotation)
					add_point(0, cell_data.ay, 0.5, 0, 1, cell_data.rotation)
					add_point(1, edge_dy, 0.5, 1, 0, cell_data.rotation)

					# Lower floor
					start_floor()
					add_point(0, cell_data.cy, 0.5, 1, 0, cell_data.rotation)
					add_point(1, edge_dy, 0.5, 1, 0, cell_data.rotation)
					add_point(0, cell_data.cy, 1, 0, 0, cell_data.rotation)

					add_point(1, cell_data.dy, 1, 0, 0, cell_data.rotation)
					add_point(0, cell_data.cy, 1, 0, 0, cell_data.rotation)
					add_point(1, edge_dy, 0.5, 0, 0, cell_data.rotation)
				...
				elif cell_data.ab and cell_data.ac and cell_data.cd and cell_data.is_higher(cell_data.by, cell_data.dy):
					# Only merge the ay/cy edge if AC edge is connected
					var edge_ay = (cell_data.ay+cell_data.cy)/2
					var edge_cy = (cell_data.ay+cell_data.cy)/2
					
					# Upper floor - use A and B edge for heights
					start_floor()
					add_point(0, cell_data.ay, 0, 0, 0, cell_data.rotation)
					add_point(1, cell_data.by, 0, 0, 0, cell_data.rotation)
					add_point(0, edge_ay, 0.5, 0, 0, cell_data.rotation)
					
					add_point(1, cell_data.by, 0.5, 0, 1, cell_data.rotation)
					add_point(0, edge_ay, 0.5, 0, 1, cell_data.rotation)
					add_point(1, cell_data.by, 0, 0, 0, cell_data.rotation)
					
					# Wall from left to right edge
					start_wall()
					add_point(1, cell_data.by, 0.5, 1, 1, cell_data.rotation)
					add_point(1, cell_data.dy, 0.5, 1, 0, cell_data.rotation)
					add_point(0, edge_ay, 0.5, 0, 0, cell_data.rotation)
					
					# Lower floor - use C and D edge
					start_floor()
					add_point(0, edge_cy, 0.5, 1, 0, cell_data.rotation)
					add_point(1, cell_data.dy, 0.5, 1, 0, cell_data.rotation)
					add_point(1, cell_data.dy, 1, 0, 0, cell_data.rotation)
					
					add_point(0, cell_data.cy, 1, 0, 0, cell_data.rotation)
					add_point(0, edge_cy, 0.5, 0, 0, cell_data.rotation)
					add_point(1, cell_data.dy, 1, 0, 0, cell_data.rotation)				
...
func add_point(x: float, y: float, z: float, uv_x: float, uv_y: float, ro: CellData.CellRotation, diag_midpoint: bool = false):
...
func add_full_floor(dat: CellData):
	start_floor()
	
	if (higher_poly_floors):
		var ey = (dat.ay+dat.by+dat.cy+dat.dy)/4

		add_point(0, dat.ay, 0, 0, 0, dat.rotation)
		add_point(1, dat.by, 0, 0, 0, dat.rotation)
		add_point(0.5, ey, 0.5, 0, dat.rotation, dat.rotation, true)
		
		add_point(1, dat.by, 0, 0, 0, dat.rotation)
		add_point(1, dat.dy, 1, 0, 0, dat.rotation)
		add_point(0.5, ey, 0.5, 0, 0, dat.rotation, true)
		
		add_point(1, dat.dy, 1, 0, 0, dat.rotation)
		add_point(0, dat.cy, 1, 0, 0, dat.rotation)
		add_point(0.5, ey, 0.5, 0, 0, dat.rotation, true)
...
func add_outer_corner(dat: CellData, floor_below: bool = true, floor_above: bool = true, flatten_bottom: bool = false, bottom_height: float = -1):
	var edge_by = bottom_height if flatten_bottom else dat.by
	var edge_cy = bottom_height if flatten_bottom else dat.cy
	
	if floor_above:
		start_floor()
		add_point(0, dat.ay, 0, 0, 0, dat.rotation)
		add_point(0.5, dat.ay, 0, 0, 1, dat.rotation)
		add_point(0, dat.ay, 0.5, 0, 1, dat.rotation)
	
	# Walls - bases will use B and C height, while cliff top will use A height.
	start_wall()
	add_point(0, edge_cy, 0.5, 0, 0, dat.rotation)
	add_point(0, dat.ay, 0.5, 0, 1, dat.rotation)
	add_point(0.5, edge_by, 0, 1, 0, dat.rotation)
	
	add_point(0.5, dat.ay, 0, 1, 1, dat.rotation)
	add_point(0.5, edge_by, 0, 1, 0, dat.rotation)
	add_point(0, dat.ay, 0.5, 0, 1, dat.rotation)
	...
	if floor_below:
		...
		add_point(0, dat.cy, 1,0,0, dat.rotation)
		add_point(0, dat.cy, 0.5, 1, 0, dat.rotation)
		add_point(0.5, dat.by, 0, 1, 0, dat.rotation)
...
func add_edge(dat: CellData, floor_below: bool, floor_above: bool, a_x: float = 0, b_x: float = 1):
	# If A and B are out of merge distance, use the lower of the two
	var edge_ay = dat.ay if dat.ab else min(dat.ay, dat.by)
	var edge_by = dat.by if dat.ab else min(dat.ay, dat.by)
	var edge_cy = dat.cy if dat.cd else max(dat.cy, dat.dy)
	var edge_dy = dat.dy if dat.cd else max(dat.cy, dat.dy)
	
	# Upper floor - use A and B for heights
	if floor_above:
		start_floor()
		add_point(a_x, edge_ay, 0, 1 if a_x > 0 else 0, 0, dat.rotation)
		add_point(b_x, edge_by, 0, 1 if b_x < 1 else 0, 0, dat.rotation)
		add_point(0, edge_ay, 0.5, -1 if b_x < 1 else (1 if a_x > 0 else 0), 1, dat.rotation)
		
		add_point(1, edge_by, 0.5, -1 if a_x > 0  else (1 if b_x < 1 else 0), 1, dat.rotation)
		add_point(0, edge_ay, 0.5, -1 if b_x < 1 else (1 if a_x > 0 else 0), 1, dat.rotation)
		add_point(b_x, edge_by, 0, 1 if b_x < 1 else 0, 0, dat.rotation)
	...
	start_wall()
	add_point(0, edge_cy, 0.5, 0, 0, dat.rotation)
	add_point(0, edge_ay, 0.5, 0, 1, dat.rotation)
	add_point(1, edge_dy, 0.5, 1, 0, dat.rotation)
	
	add_point(1, edge_by, 0.5, 1, 1, dat.rotation)
	add_point(1, edge_dy, 0.5, 1, 0, dat.rotation)
	add_point(0, edge_ay, 0.5, 0, 1, dat.rotation)
	...
	if floor_below:
		start_floor()
		add_point(0, dat.cy, 0.5, 1, 0, dat.rotation)
		add_point(1, dat.dy, 0.5, 1, 0, dat.rotation)
		add_point(0, dat.cy, 1, 0, 0, dat.rotation)
...
func add_inner_corner(dat: CellData, lower_floor: bool = true, full_upper_floor: bool = true, flatten: bool = false, bd_floor: bool = false, cd_floor: bool = false):
	var corner_by = min(dat.by, dat.cy) if flatten else dat.by
	var corner_cy = min(dat.by, dat.cy) if flatten else dat.cy
	
	# Lower floor with height of point A
	if lower_floor:
		start_floor()
		add_point(0, dat.ay, 0, 0, 0, dat.rotation)
		add_point(0.5, dat.ay, 0, 1, 0, dat.rotation)
		add_point(0, dat.ay, 0.5, 1, 0, dat.rotation)

	start_wall()
	add_point(0, dat.ay, 0.5, 1, 0, dat.rotation)
	add_point(0.5, dat.ay, 0, 0, 0, dat.rotation)
	add_point(0, corner_cy, 0.5, 1, 1, dat.rotation)
	
	add_point(0.5, corner_by, 0, 0, 1, dat.rotation)
	add_point(0, corner_cy, 0.5, 1, 1, dat.rotation)
	add_point(0.5, dat.ay, 0, 0, 0, dat.rotation)
	...
	if full_upper_floor:
		add_point(1, dat.dy, 1, 0, 0, dat.rotation)
		add_point(0, corner_cy, 1, 0, 0, dat.rotation)
		add_point(1, corner_by, 0, 0, 0, dat.rotation)
		
		add_point(0, corner_cy, 1, 0, 0, dat.rotation)
		add_point(0, corner_cy, 0.5, 0, 1, dat.rotation)
		add_point(0.5, corner_by, 0, 0, 1, dat.rotation)
	...
	if cd_floor:
		...
		add_point(1, dat.by, 0, 0, 0, dat.rotation)
		add_point(0, dat.by, 0.5, 1, 1, dat.rotation)
		add_point(0.5, dat.by, 0, 0, 1, dat.rotation)
		
		add_point(1, dat.by, 0, 0, 0, dat.rotation)
		add_point(1, dat.by, 0.5, 1, -1, dat.rotation)
		add_point(0, dat.by, 0.5, 1, 1, dat.rotation)
	...
	if bd_floor: 
		add_point(0, dat.cy, 0.5, 0, 1, dat.rotation)
		add_point(0.5, dat.cy, 0, 1, 1, dat.rotation)
		add_point(0, dat.cy, 1, 0, 0, dat.rotation)
		
		add_point(0.5, dat.cy, 1, 1, -1, dat.rotation)
		add_point(0, dat.cy, 1, 0, 0, dat.rotation)
		add_point(0.5, dat.cy, 0, 1, 1, dat.rotation)


# Add a diagonal floor, using heights of B and C and connecting their points using passed heights.
func add_diagonal_floor(b_y: float, c_y: float, rot: CellData.CellRotation, a_cliff: bool, d_cliff: bool):
	start_floor()
	
	add_point(1, b_y, 0, 0 ,0 ,rot)
	add_point(0, c_y, 1, 0 ,0 ,rot)
	add_point(0.5, b_y, 0, 0 if a_cliff else 1, 1 if a_cliff else 0, rot)
	
	add_point(0, c_y, 1, 0, 0, rot)
	add_point(0, c_y, 0.5, 0 if a_cliff else 1, 1 if a_cliff else 0, rot)
	add_point(0.5, b_y, 0, 0 if a_cliff else 1, 1 if a_cliff else 0, rot)
	
	add_point(1, b_y, 0, 0 ,0 ,rot)
	add_point(1, b_y, 0.5, 0 if d_cliff else 1, 1 if d_cliff else 0, rot)
	add_point(0, c_y, 1, 0 ,0 ,rot)
	
	add_point(0, c_y, 1, 0, 0, rot)
	add_point(1, b_y, 0.5, 0 if d_cliff else 1, 1 if d_cliff else 0, rot)
	add_point(0.5, c_y, 1, 0 if d_cliff else 1, 1 if d_cliff else 0, rot)
